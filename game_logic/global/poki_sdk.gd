...
func commercialBreak(on_start = null):
	...
	if _is_valid_callable(on_start):
		promise = sdk_handle.commercialBreak(
			_create_js_callback(
				Callable(self, "_invoke_no_arg_callable").bind(on_start),
				callback_id,
			)
		)
	else:
...
func displayAd(container, size = null, on_can_destroy = null, on_display_rendered = null):
	...
	if _is_valid_callable(on_can_destroy):
		args.append(
			_create_js_callback(
				Callable(self, "_on_display_ad_can_destroy").bind(
					on_can_destroy,
					callback_id,
					container_key,
				),
	...
	sdk_handle.displayAd(
		args[0],
		args[1],
		args[2] if args.size() > 2 else null,
		args[3] if args.size() > 3 else null,
	)
...
func _attach_promise_with_args(
	...
	var resolve_callback = _create_js_callback(
		Callable(self, "_on_promise_resolved").bind(
			success_signal,
			callback_id,
			emit_payload,
			transformer,
...
func _normalize_user(user):
	...
	return { "username": str(user.username), "avatarUrl": str(user.avatarUrl) }
